...
npm run test:consistency # Golden tests สำหรับ src/lib/consistencyCheck.js — ตรวจความสอดคล้อง CSV→DB (40 assertions)
npm run test:countmatch  # Golden tests สำหรับ src/lib/countMatch.js — ตรวจนับ: 3 สถานะต่อมิติ ครบ 4 มิติ (จำนวน/ที่เก็บ/exp/lot) + set equality ที่เก็บ/exp (49 assertions)
...
- **ฟอร์มพิมพ์ในเมนู "แบบฟอร์มต่างๆ" = family เดียวกัน 10 ใบ ใน 5 ไฟล์ (เมนูแสดง 9 — ฟอร์มคืนยาใกล้หมดอายุถอดออกแล้ว แต่โค้ดยังอยู่)** — ทุกใบเป็น *กระดาษเปล่าให้กรอกมือ ไม่ดึงข้อมูลจากระบบ* เรียกผ่าน `action` item → `runFormAction` ([AppRoot.jsx](src/AppRoot.jsx)) → `openPrintView` ([openPrintView.js](src/lib/openPrintView.js) — Blob URL + fallback WebView, Critical Rule #4):
  - [inspectWorksheet.js](src/lib/inspectWorksheet.js) — ฟอร์มตรวจรับ (แนวนอน)
  - [returnForm.js](src/lib/returnForm.js) — ฟอร์มคืนยาภายใน รพ. · ฟอร์มคืนยาใกล้หมดอายุ (ตัวหลังถอดจากเมนูแล้ว ใบสำรวจยาหมดอายุครอบคลุมกว่า — โค้ดยังอยู่ ไม่ได้ลบ)
  - [vendorExchangeCycle.js](src/lib/vendorExchangeCycle.js) — ใบส่งคืนบริษัท (ขาออก) · ใบรับยาเปลี่ยนกลับ (ขาเข้า) · โหมดพิมพ์คู่ 2 แผ่น. ผูกกันด้วย **เลขที่รอบ VX** (ADR-0024)
  - [expiryDispositionForm.js](src/lib/expiryDispositionForm.js) — ใบสำรวจยาหมดอายุ (คัดแยก A/B/C) · ใบส่งทำลาย (กรรมการ 3 คน) ตามระเบียบฯ ข้อ 215
  - [drugLoanForm.js](src/lib/drugLoanForm.js) — ใบยืมยา · ใบคืนยา ตามระเบียบฯ ข้อ 207 (ADR-0025)
  - **กฎร่วมของทุกใบ** (เครื่องพิมพ์คลังเป็นขาวดำ): สี `#000` ล้วน ห้ามเทา · `th` พื้นขาว ใช้ตัวหนาแยกหัวตาราง · `@page` A4 **portrait** · **ช่องเว้นให้เขียนมือต้องมี `height` + `vertical-align: bottom`** (class `.fl`) ไม่งั้นเส้นลอยไปกลางบรรทัดเหมือนขีดฆ่า เขียนทับไม่ได้ — และ flex container ที่ครอบต้องเป็น `align-items: baseline` ไม่ใช่ `center`
  - **เพิ่มฟอร์มใหม่ต้อง sync 3 ที่**: `action` ใน `NAV_GROUPS` ([navConfig.js](src/navConfig.js)) + import & dispatch ใน `runFormAction` ([AppRoot.jsx](src/AppRoot.jsx)) + ไฟล์ฟอร์มเอง
...
- `UserManagementApp.jsx` — admin จัดการ user — ตาราง 7 คอลัมน์ (ผู้ใช้ avatar+ชื่อ / หน่วยงาน / ประเภท / สิทธิ์ / สถานะ / วันที่สมัคร / จัดการ). **คอลัมน์สิทธิ์แสดง "สรุป" ไม่ใช่ไล่ชื่อทุกระบบ** — `summarizeAccess()` นับจาก `GRANTABLE_SYSTEMS.defaultRoles` (แหล่งเดียวกับโมดอลสิทธิ์ ตัวเลขในตารางกับติ๊กในโมดอลจึงตรงกันเสมอ) → ป้าย `สิทธิ์มาตรฐาน (N ระบบ)` / `ทุกระบบ` (admin) + ป้ายเส้นประ `+<ระบบ>` เฉพาะที่ grant เพิ่ม. **เพิ่มระบบใหม่แก้ที่ `GRANTABLE_SYSTEMS` ที่เดียว** (เดิมมีลิสต์ `SYSTEM_ACCESS` พิมพ์มือแยก ที่ drift จนขาด analytics/ledger/stockcard — ลบทิ้ง 2026-08-01). action ต่อแถว = ปุ่ม "แก้ไข" นอก + เมนู ⋮ (สิทธิ์/รหัสผ่าน/ลบ — ลบอยู่ล่างสุดแยกเส้นคั่น กันกดผิด)
...
- `StockCountApp.jsx` — ตรวจนับคงคลัง (staff/admin) — เลือกรหัสยา → ได้ lot ที่ qty>0 (lot ที่ระบบว่า 0 ซ่อน default กด "แสดงเพื่อนับ" ได้ — phantom stock) แต่ **ติ๊กเลือก lot ที่จะนับ** (`_selected` default true; save/print เฉพาะที่ติ๊ก; ปุ่ม X = `removeDrug` เอายาออกทั้งตัว ≠ checkbox ข้าม lot รายตัว) → นับ **4 มิติ** (จำนวน/ที่เก็บ/exp/**lot**) เทียบ `inventory.qty` สด. **ต้องตรวจครบทุกมิติที่ระบบมีค่าก่อน save** (24/09/2569 — `missingDims()` ใน countMatch.js ใช้ร่วม 3 จุด: `handleSave` เฉพาะจุด / `saveDraft` ประจำปี / `saveEdit` ในประวัติ; มิติที่ระบบเป็น `-` ไม่บังคับ — lot เวชภัณฑ์ 39 lot + ไม่มี exp 37 lot จะบันทึกไม่ได้ตลอดกาลถ้าบังคับ; หน้าเฉพาะจุดเพิ่มช่อง lot + `createStockCount` บันทึก `counted_lot` แล้ว — เดิมไม่มีทั้งคู่); มิติว่าง = "ไม่ได้ตรวจ" ไม่ใช่ "ตรง" — logic เทียบทั้งหมดอยู่ใน pure module **`src/lib/countMatch.js`** (`dimStatus`/`computeCountMatch`/`diffLabel`; ที่เก็บ/exp เทียบแบบ **set equality** split comma ไม่สนลำดับ — แต่ **`counted_lot` เทียบตรงตัว** trim + ไม่สนพิมพ์ใหญ่เล็ก **ห้ามใช้ `setEq`** เพราะ lot เป็นรหัสเดียว comma ในชื่อไม่ใช่ตัวคั่น; `counted_lot` ว่าง = ไม่ได้ตรวจ คนละคอลัมน์กับ `lot` ที่เป็น snapshot ของระบบ). **ช่อง "ที่เก็บจริง" = `LocationInput`** (พิมพ์เองได้ + เลือก suggestion ซ้ำ = append ต่อท้ายคั่น comma). วันที่รอบ (`counted_at`) เลือกได้ตอนนับ + แก้ได้ในประวัติ (ปุ่มดินสอบนหัวรอบ → `updateStockCountSession`). **draft กันงานหาย**: เก็บ localStorage ต่อ user (`stockcount_draft_<username>`) + banner กู้คืน/ทิ้ง (ไม่ auto-restore). append-only เก็บ `stock_count_session`+`stock_count_item` (freeze snapshot, ไม่แก้ qty) + พิมพ์ใบเดินนับ (มี LINE WebView fallback แล้ว). ประวัติ: badge "ไม่ตรง N รายการ"/"ตรงทั้งหมด" (เฉพาะตรวจครบทุกมิติ)/"ตรงตามที่ตรวจ" บนหัวรอบ (จาก `allItems` โหลดครบทุกรอบ), ส่วนต่างแสดง "ขาด N/เกิน N", ค้นยา → **timeline ทุกครั้งที่เคยนับ** + chip "นับล่าสุด" ในหน้านับ (ดู [ADR-0008](docs/adr/0008-stock-count-append-only-snapshot.md))
  - **รอบตรวจนับประจำปี (`kind='annual'`)** — แยกจาก spot check ด้วย `stock_count_session.kind` (`'spot'`|`'annual'`): ระบบ gen ทุก lot ที่มีของ (~632 บรรทัด) แล้วไล่นับ**ตามชั้นวางบนลงล่าง** ไม่ใช่เลือกยาเอง. **draft อยู่บน DB ไม่ใช่ localStorage** (ต่างจาก spot check) เพราะงานกินเวลาหลายวัน ข้ามเครื่อง/ข้ามวัน — `status='draft'` ระหว่างนับ → `'done'` ตอนปิดรอบ; `fetchOpenAnnualCount()` หา draft ค้างตอนเปิดหน้า. บันทึกอัตโนมัติรายบรรทัดด้วย `updateAnnualCountLine()` ซึ่ง **ไม่เขียน audit log โดยเจตนา** (632 แถวจะท่วมกระดิ่ง) — เขียนเฉพาะระดับรอบ: `createAnnualCount`/`closeAnnualCount`/`addLotToAnnualCount`/`addUnknownItemToAnnualCount`. เจอของที่ไม่มีในระบบระหว่างนับ → `addUnknownItemToAnnualCount` (action `add_unknown_count_item`). migration: `stock_count_followup_migration.sql`
  - **ประวัติแสดงเฉพาะ "บรรทัดที่นับแล้ว"** (มี `counted_qty`) — บรรทัดที่ระบบ gen รอไว้คือ*งานที่เหลือ* ไม่ใช่ผลการตรวจ. สลับดู "ทั้งหมด" ได้ต่อรอบ และ **Excel ออกตามที่ตารางแสดงอยู่** (Critical Rule #6) แต่ **ใบรับรองล็อกที่ "นับแล้ว" เสมอ ไม่ตามตัวสลับ**. กรอง 3 จุด: ตาราง+timeline ([StockCountApp.jsx](src/StockCountApp.jsx)) + `lastCounted` ใน `fetchCountPriorityData` ([db.js](src/lib/db.js) — ไม่กรองแล้วยา 333 รหัสที่ไม่เคยนับจะกลายเป็น "เพิ่งนับวันนี้" แล้วหล่นจากอันดับควรนับก่อน). ดู [ADR-0026](docs/adr/0026-count-certificate-from-counted-lines.md)
  - **ใบรับรองผลตรวจนับ** — [stockCountCertificate.js](src/lib/stockCountCertificate.js) `printCountCertificate(session, allItems, { printedBy })` ส่ง **allItems ทั้งรอบ ไม่ต้องกรองมาก่อน** (ฟังก์ชันกรองเอง + ต้องใช้ตัวส่วนไปแสดง "ยังไม่ได้นับ N"). เลขที่ `SC-<session id>`, ลายเซ็น 2 ช่อง (เจ้าหน้าที่คลัง + หัวหน้าคลัง), หัวกระดาษเปลี่ยนตาม `status` (draft = "บันทึกผล…" / done = "ใบรับรอง…"), คอลัมน์ "มิติที่ตรวจ N/4" + `item_note`. ใช้ `openPrintView` (Rule #4) — **`printCountSheet` เดิมยัง copy Blob+fallback เอง ยังไม่ย้ายมาใช้ helper กลาง**
  - **ป้าย "รอตรวจรับ N วัน" บนการ์ดนับ** — `fetchPendingReceiveLots()` ([db.js](src/lib/db.js)) คืน map **key `code|lot`** (ห้าม lot เดี่ยว — lot `-` ของเวชภัณฑ์ซ้ำข้ามตัวยา) → `{ billNumber, receiveDate, waitDays }`. ของที่มาถึงชั้นแล้วแต่ยังไม่ผ่านตรวจรับ คนนับต้องรู้ ไม่งั้นเดินตามหา "ของเกิน" ที่จริงคือของรอตรวจรับ. `waitDays` ใช้สูตรเดียวกับ `fetchDashboardAlerts` (receive_date ล่าสุดของ code+lot เทียบวันนี้) เลขจึงตรงกับ Dashboard
  - **ค้นชื่อยาในรอบนับ** — `DrugSearchBar` ในแถบตัวกรอง (โซน/ชั้น/ค้นหา) ค้นได้ทั้ง **ชื่อยา / รหัส / lot** ผ่าน `matchQ()` ที่ต่อเข้า `inScope` จุดเดียว (คิว+ตัวนับ "เหลือเท่าไหร่" จึงตรงกันอัตโนมัติ). options มาจาก `items` ของรอบเอง (ไม่ query เพิ่ม) + badge ชนิดยาจาก `fetchInventoryNameCodeMap().typeByName`. จำค่าไว้ใน localStorage ต่อรอบเหมือนตัวกรองอื่น
  - **กรอกนับแยกรายชั้น (lot ที่แบ่งเก็บหลายที่)** — `curSplit` = `locBreakdown[code|lot]` (คำนวณสดจาก inventory); ถ้า > 1 ที่เก็บ จะมีกล่องส้ม "กรอกแยกที่เก็บ" ให้ใส่ยอดรายชั้น → ปุ่ม "ใช้ยอดรวมนี้" เอาผลรวมลง `counted_qty` **ช่องเดียวเหมือนเดิม** + เขียนที่มาลง `item_note` อัตโนมัติโดยติดป้าย **`แยกชั้น: `** (`"แยกชั้น: E-1-1 ,E-1-2 ,E-1-3 550 + ห้องประชุมเล็กชั้น4 750"` — รูปแบบที่คลังเขียนมือมาก่อนแล้ว). **ไม่แตะ schema** — 1 บรรทัด = 1 (code+lot) ตาม ADR-0008 ข้อ 3; ยอดแยกรายชั้นเป็น *วิธีช่วยคำนวณ + หลักฐานข้อความ* ไม่ใช่มิติใหม่. ⚠️ **ป้าย `แยกชั้น: ` มีไว้ให้ `stripSplitNote()` แทนที่ชุดเดิมตอนกดแก้ซ้ำ** ไม่งั้นหมายเหตุมี 2 ชุดยอดขัดกันเองบนใบรับรอง — และต้องแยกออกจากข้อสังเกตที่คนพิมพ์เอง (ห้ามทับของคน). state ล้างทุกครั้งที่เปลี่ยน lot. ปัจจุบันมี lot เดียวทั้งคลังที่เข้าเคส (Manidipine 20mg lot 260873)
  - **ปุ่ม "ตรงตามระบบ" ต้องส่ง `item_note` ด้วยเสมอ** — `saveLine` ทำ `{...cur, ...fields}` ซึ่ง `cur.item_note` เป็นค่าจาก **DB** ไม่ใช่ที่กำลังพิมพ์ ถ้า `markSame` ไม่ส่ง `item_note` ไปด้วย **หมายเหตุ/ยอดแยกชั้นที่เพิ่งกรอกหายเงียบ** (เส้นทางที่เกิดง่ายมากเพราะยอดตรงระบบพอดี คนจะกดปุ่มเขียวโดยธรรมชาติ — แก้ 2026-09-22)
  - **ตัวกรองสถานะนับ = dropdown 3 ค่า** (`countFilter`: `pending` | `counted` | `all`) เดิมเป็นปุ่ม toggle 2 สถานะซึ่งบอกไม่ได้ว่า "ดูเฉพาะที่นับแล้ว" ทำได้. ใช้ 4 จุด: `queue` / เลื่อนตัวถัดไปใน `saveLine` (โหมด `pending` แถวหลุดคิวเอง ไม่ต้องขยับ) / localStorage / empty state. **restore รองรับค่าเก่า `onlyPending` (boolean)** ที่ค้างใน localStorage ของรอบที่นับค้างอยู่ — ไม่แปลงให้ ตัวกรองจะรีเซ็ตเองตอนเปิดมา
  - **E2E**: [tests/14-stockcount-split.spec.js](tests/14-stockcount-split.spec.js) — 7 เทส (ค้นหา/กล่องแยกชั้น/รวมยอด/กดแก้ซ้ำ/หมายเหตุไม่หาย/dropdown). ⚠️ **read-only ทั้งหมด ไม่กดบันทึก** (รอบประจำปีเป็นข้อมูลจริงของคลัง) + `test.skip` ถ้าไม่มีรอบ draft ค้าง
  - **ปุ่ม "รีเฟรชยอดระบบ"** — `refreshAnnualCountSystemQty(sessionId, auth)` ([db.js](src/lib/db.js)) ดึง `inventory` ปัจจุบันมาอัปเดต `system_qty/exp/location` **เฉพาะบรรทัดที่ยังไม่ได้นับ** ในรอบที่ยัง `draft`. จำเป็นเพราะรอบประจำปีกินเวลาหลายสัปดาห์ ระหว่างนั้นมีเบิกจ่าย + import Master ทุกวัน → บรรทัดที่ยังไม่นับถือยอดของ "วันเปิดรอบ" ไม่ใช่ยอดที่ควรเทียบตอนเดินนับ (เหตุการณ์ 21/09/2569: Oral ผู้ใหญ่ รอบถือ 80 ระบบจริง 56 เพราะเบิกออก 24 — จะรายงาน "ขาด 24" ทั้งที่ของครบ). **ห้ามแตะบรรทัดที่นับแล้ว** (บังคับ 2 ชั้น: filter ตอน select + `.is('counted_qty', null)` ซ้ำใน update กัน race) — นั่นคือ snapshot ที่คู่กับผลนับ แก้แล้วผิด ADR-0008 ของจริง. audit `refresh_annual_count_qty`. ดู [ADR-0008 เพิ่มเติม 2026-09-22](docs/adr/0008-stock-count-append-only-snapshot.md)
  - **เวลานับรายบรรทัด (`stock_count_item.counted_at`)** — รอบประจำปีกินเวลาหลายวัน หัวรอบบอกแค่*วันเปิดรอบ* จึงตอบไม่ได้ว่ายาตัวไหนนับวันไหน. `updateAnnualCountLine` stamp เวลา **เฉพาะครั้งแรกที่มียอดนับ** (`.is('counted_at', null)` + เช็ค `counted_qty != null`) — แก้ยอดทีหลังไม่ทับ (ADR-0008). ⚠️ **ต้องเป็น UPDATE คำสั่งที่ 2 แยกจากการบันทึกผลนับ** ห้ามยัด guard ลงคำสั่งแรก ไม่งั้นบรรทัดที่เคยนับแล้วจะถูกปฏิเสธทั้งคำสั่ง → **ยอดที่แก้ใหม่หายเงียบ** (`saveLine` เป็น optimistic ขึ้น "saved" ทั้งที่ DB ไม่เปลี่ยน). ⚠️ ใช้นาฬิกา **client** เพราะ supabase-js ส่ง `NOW()` ใน UPDATE ไม่ได้ และ repo ไม่เคยใช้ `rpc()` เลย — ตรงกับ `updated_at`/`received_at` ตัวอื่นใน db.js (ต่างจาก `created_at` ที่เป็น `DEFAULT NOW()` ฝั่ง DB เพราะ DEFAULT ทำงานเฉพาะตอน INSERT). `clearStockCountItem` ต้องล้างคอลัมน์นี้ด้วย. **แถวที่นับก่อน 24/09/2569 ไม่มีเวลา — แสดงว่าง ห้าม fallback ไปวันเปิดรอบ** (ข้อมูลปลอมที่ดูเหมือนจริง). แสดง 3 จุด: ใต้ชื่อยาในตารางรอบ · คอลัมน์ "นับเมื่อ" ใน timeline ผลค้นหา (เดิมโชว์ `session.created_at` = บั๊ก ทุกแถวเวลาเท่ากันหมด) · Excel. **ไม่แตะใบรับรอง** (ADR-0026 ล็อกที่ "นับแล้ว"). migration: `stock_count_followup_migration.sql`
  - **ล้างผลนับรายบรรทัด** — `clearStockCountItem(itemId, auth)` ([db.js](src/lib/db.js)) กลับเป็น "ยังไม่ได้นับ" (`counted_qty = null`) **แถวยังอยู่** ไม่ใช่ลบหลักฐาน (append-only ADR-0008). ⚠️ **ต้องล้าง `counted_lot` ด้วย** — `updateStockCountItem` ไม่เคยแตะคอลัมน์นี้ (มีแต่ `updateAnnualCountLine` ที่เขียน) ไม่ล้างแล้ว lot ค้างกับแถวที่ว่าง. ไม่แตะ `followup_*` (คนละเรื่อง ADR-0017). audit action `clear_stock_count_item` (sync ครบ 4 ที่ตาม Rule #12)
...
| ยืม-คืนยา: ของที่ส่งต่อผู้ดูแลไม่นับเป็นของค้างคืน | [ADR-0025](docs/adr/0025-pass-through-loan-closed-on-handover.md) · [CONTEXT.md §ของค้างคืน](CONTEXT.md) | แก้ `DrugLoanApp` / นิยาม "ค้างคืน" / ฟอร์มยืม-คืนยา |
...
- **`printTrackingList(rows, { title, isExpiryMode, filterNote, printedBy })`** ([trackingPrint.js](src/lib/trackingPrint.js)) — พิมพ์รายการจากโมดอลติดตามในแผนผังคลังยา (รอตรวจรับ / ใกล้หมดอายุ / หมดอายุแล้ว), A4 แนวนอน + fallback WebView ครบ. **ต้องส่ง `timeFiltered` ชุดเดียวกับที่ตาราง/Excel ใช้** (Rule #6) + `filterNote` บอกตัวกรองที่ใช้บนหัวกระดาษ ไม่งั้นกระดาษที่พิมพ์ตอนกรองอยู่ดูเหมือนรายการทั้งหมด. คอลัมน์ล้อ Excel export ของโมดอลเดียวกัน — `isExpiryMode` ตัด บิลยา/PO/วันที่รับเข้า/รอตรวจรับมาแล้ว ออก (ไม่มีความหมายกับของที่ตรวจรับแล้ว). "สถานะตรวจรับ" ตัดส่วนหลัง `|` (`receiveStatus` เก็บรวมเป็น `<สถานะ>|<ผลการพิจารณา>`)
...
- **`importDrugLoans({ inserts, updates, deleteIds, fileName }, auth)`** (db.js) — นำเข้าไฟล์ CSV ยืม-คืนยา **ไม่ใช่ DELETE-then-INSERT** (ต่างจาก `insertReceiveRows`/import ชีทเบิก): รับ "แผน" ที่ `diffLoanImport` ([loanImport.js](src/lib/loanImport.js)) คำนวณมาแล้ว → insert เฉพาะแถวใหม่ + update เฉพาะช่องที่ไฟล์เปลี่ยน (แถวเดิมคง `id`/`note` ที่พิมพ์ในแอป) + ลบเฉพาะ id ที่ผู้ใช้ติ๊กเอง. **key จับคู่แถวต้องตรงกับ unique index `drug_loan_dedupe_key`** ใน [drug_loan_migration.sql](drug_loan_migration.sql) เป๊ะๆ (`direction|counterparty|drug_code|lot|loan_date|qty`) ไม่งั้น import สร้างแถวที่ DB ปฏิเสธ. ทิศทาง (borrow/lend) **derive จากคอลัมน์ "รพ.ที่ขอยืม" vs "รพ.ที่ให้ยืม"** เทียบกับชื่อ รพ.เรา — ไม่มีชื่อ รพ.เราสักฝั่ง = แถวเสีย ไม่เดา. audit action `import_drug_loan`
...
12. **Notification & AuditLog sync**: ถ้าเพิ่ม action ใหม่ใน `insertAuditLog` ต้องเพิ่ม label ใน **3 ที่พร้อมกัน**: `NOTIF_LABELS` ([NotificationBell.jsx](src/NotificationBell.jsx) — ย้ายมาจาก AppRoot แล้ว), `NOTIFY_ACTIONS` ใน `fetchNotifications` ([db.js](src/lib/db.js)), `ACTION_LABELS` ([AuditLogApp.jsx](src/AuditLogApp.jsx)) — ไม่งั้นปุ่ม bell ไม่ขึ้น หรือ UI แสดง raw key. **กระดิ่ง = `NotificationBell.jsx` วางบน top bar (แถบสีฟ้า) ของ [AppShell.jsx](src/AppShell.jsx) คู่ user chip → แสดงทุกหน้า ทุก role** (top bar ไม่ sticky โดยเจตนา — กันชน sticky header ของ sub-app). **scope**: staff/admin = global feed; requester = เฉพาะแผนกตัวเอง (match `department` OR `details.req_department` — lifecycle ฝั่ง staff ต้อง stamp `req_department` ตอน log ไม่งั้น requester ไม่เห็นใบเบิกตัวเอง ดู ADR-0010 + CONTEXT.md §"การแจ้งเตือนในแอป")
   - ⚠️ **ตรวจพบ drift 2026-09-23**: `NOTIF_LABELS` มี 50 key · `NOTIFY_ACTIONS` (db.js) 47 · `ACTION_LABELS` 81 — **`delete_drug_loan` กับ `add_annual_count_lot` มี label ในกระดิ่ง + เขียน audit จริง แต่ `NOTIFY_ACTIONS` ไม่ได้ query → ไม่เด้งกระดิ่งตลอดกาล** (`update_drug_loan` มี label แต่ยังไม่มีใครเรียก = label เปล่า). **ทิศทาง drift ที่รูลเดิมไม่ได้เตือน**: ลืมเติม `NOTIFY_ACTIONS` แล้วเงียบสนิท ไม่ error. เช็คเร็ว: extract key ทั้ง 3 ที่มา diff กัน ก่อนเชื่อว่า sync ครบ
...
- **อย่าแปลง scientific notation กับ `lot` — lot เป็น "รหัส" ไม่ใช่จำนวน**: lot จริงรูปแบบ `<ปี><เดือนเป็นตัวอักษร><ลำดับ>` (`26E266`, `26D172`, `26F116`) ซึ่ง regex `^[\d.]+[eE][+-]?\d+$` มองว่าเป็น 26×10²⁶⁶ → `parseFloat` ปัดด้วย float64 → `BigInt` ขยายเป็นเลข **269 หลัก** lot หายถาวร. ใช้ `normalizeLot()` ([App.jsx](src/App.jsx)) ที่เก็บตามที่พิมพ์มา — **`normalizeNumericText()` ใช้ได้เฉพาะ `invoice`/`qtyReceived`** (เลขที่บิลแบบ `1.12512E+11` ต้องขยายจริง). เสียเฉพาะ lot ที่มีตัว **E** (D/F รอด เพราะไม่ใช่สัญลักษณ์ exponent). **CSV ต้นทางไม่ได้พัง — โค้ด import เป็นคนทำพัง** (เหตุการณ์ 2026-07-29: เสีย 6 แถว, กู้จาก `receive_logs` ด้วย `code+invoice+exp`, backup `_bak_inventory_lot_20260731`; แก้ 2026-08-01)
